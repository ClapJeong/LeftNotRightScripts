...
{
  public class UIGlobalRecordPresenter : IUIPresenter
  {
    public class Model
    {
    ...
    private readonly UIGlobalRecordView view;
    ...
    public UIGlobalRecordPresenter(Model model, UIGlobalRecordView view)
    {
      this.model = model;
      this.view = view;    
    ...
    private async UniTask UpdateLeaderboardViewAsync(ILeaderBoardService.RankData[] datas)
    {
      ...
      for (int i = 0; i < targetSet.RecordSets.Count; i++)
      {
        ...
        if (i < datas.Length)
        {
          ...
          set.FontObject.UpdateText(-data.Score * LeaderboardUnit.Unit);
